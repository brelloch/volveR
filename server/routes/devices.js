...
module.exports = function(app) {
  ...
  // set all away
  app.route('/all_away/:id').get(function(req, res) {
    var AllUsersAway = true;
    ...
    .exec(function(err, user) {
      ...
      User.find({}, function (err, users) {
        ...
        users.forEach(function(user) {
          console.log(AllUsersAway);
          console.log(user.status);
          if (user.status === 'home' && user._id != req.params.id) {
            AllUsersAway = false;
          }
        });

        if (AllUsersAway === false) {
          res.jsonp('');
        } else {
  ...
  // set all home
  app.route('/all_home/:id').get(function(req, res) {
    console.log('id: ' + req.params.id);
    User
    .findOne({
      _id: req.params.id
  ...
  var handleDeviceOff = function(device) {
    ...
    case wemo.WemoControllee.deviceType:
      ...
      console.log('wemo state: ' + wemoSwitch.BinaryState);
